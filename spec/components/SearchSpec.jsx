describe('Search', function() {
  ...
  describe('when rendering live data from YouTube', function() {
    ...
    it('should update the video list when typing into the input box', function() {
      ...
      var searchInputElement = findRenderedDOMComponentWithClass(
        app,
        'form-control'
      );
      ...
      var newVideoEntryTitleElements = scryRenderedDOMComponentsWithClass(
        app,
        'video-list-entry-title'
      );
      newVideoEntryTitleElements.forEach((videoEntryTitle, i) => {
        expect(videoEntryTitle.innerHTML).to.equal(
          moreFakeVideoData[i].snippet.title
        );
      });
    });
  });
